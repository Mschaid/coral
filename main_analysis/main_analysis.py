...
from coral.processing_strategies import (
    ProcessingStrategy,
    BehaviorProcessingStrategy,
    PhotometryProcessingStrategy,
)
from coral.aggregators import (
    BehaviorAggregationStrategy,
    PhotometryAggregationStrategy,
    aggregate_data,
)
from coral.batch_data_processors import (
    BatchBehaviorDataPreprocessor,
    BatchPhotometryDataPreprocessor,
)
import multiprocessing as mp
import logging
...
def create_logger(log_directory):
    ...
    if not log_directory.exists():
        log_directory.mkdir()
    logging.basicConfig(
        filename=log_file_directory, level=logging.INFO, format=log_format
    )
...
def main():
    ...
    behavior_analysis(
        configs=CONFIGS,
        meta_data_factory=META_DATA_FACTORY,
        behavior_strategy_params=BEHAVIOR_STRATEGY_PARAMS,
        cpus=CPUS,
    )
    photometry_analysis(
        configs=CONFIGS,
        meta_data_factory=META_DATA_FACTORY,
        photometry_strategy_params=PHOTOMETRY_STRATEGY_PARAMS,
        cpus=CPUS,
    )
